...
/*
foreach문(개선된 for문)
: 배열의 일부가 아니라 전체를 참조해야 할때 주로 사용하는 for문을
말한다. 배열의 원소를 참조의 형태로 가져와서 사용한다.
...
public class E07ForEachBasic {
	...
	public static void main(String[] args) {
		...
		/* 배열 arr의 원소를 순서대로 하나씩 참조하여 출력하고,
		   더이상 참조할 원소가 없을때 for문이 자동으로 종료된다.*/
		sum = 0;
		for(int e : arr) {
			//배열의 원소들이 순서대로 출력된다.
			System.out.print(e+" ");
		...
		/* 참조형태로 각 원소를 읽어와서 변수 b에 할당하는 형식이므로
		   아래 증가연산자를 통해 증가시킨 값은 실제 배열에
		   적용되지 않는다. 해당 for문 내에서만 증가된 값이 출력된다.
		*/
