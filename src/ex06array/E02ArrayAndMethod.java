...
public class E02ArrayAndMethod {
	...
	public static void main(String[] args) {
		/* 배열을 선언과 동시에 초기화한다. 초기값의 갯수를 통해
		  크기가 결정된다. 생성된 배열 인스턴스의 주소값을 int형
		  배열변수인 arr에 할당하게 된다. */
		int[] arr = {1, 2, 3, 4, 5};
		int[] ref;
		...
		/*
		배열명을 인수로 전달하는 것은 배열의 참조값(주소값)을
		전달하는 것으로 배열자체가 전달되지 않는다. 아래 출력문에서는
		배열 인스턴스의 참조값(주소값)이 출력된다.
		*/
		System.out.println("arr변수:"+ arr);
		
		/* arr은 참조값이지만, 7은 기본자료형인 int이므로 값
		자체가 전달된다. 즉 복사가 일어난다. */
