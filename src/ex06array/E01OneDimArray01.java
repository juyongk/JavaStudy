...
-자바에서는 배열을 인스턴스를 통해 생성하므로 new를 사용한다.
...
public class E01OneDimArray01 {
	...
	public static void main(String[] args) {
		...
		//정수형 배열이 힙영역에 생성된 후 반환된 주소값이 출력된다.
		...
		*/
		...
		인스턴스 배열 : 기본자료형을 저장하는 배열이 아닌 인스턴스의 
			참조값을 저장할 목적으로 생성한 배열을 말한다. 사용법은
			동일하다.
		*/
		...
		//가장 기본적인 선언과 초기화 방법
		System.out.println("방법1 : 배열의 선언이후 초기화");
		//크기가 1인 정수형 배열을 선언한 뒤...
		...
		/* 배열의 선언과 동시에 초기화하는 경우 자바 컴파일러는
		 초기값의 갯수를 통해 배열의 크기를 결정할 수 있다.
		 따라서 이런 경우에는 크기가 생략하도록 약속되어 있다. */
		System.out.println("방법2 : 선언과 동시에 초기화1");
//		int[] initArr2 = new int[3] {1, 20, 300};//에러발생
		int[] initArray2 = new int[] {1, 20, 300};
		
		/* 배열 원소의 갯수를 통해 배열을 생성한 후 초기화까지
		   동시에 진행한다. 이 경우 new를 생략할 수 있다. */
