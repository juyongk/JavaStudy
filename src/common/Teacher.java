...
public class Teacher extends Person{
	...
	*/
	@Override
	public int hashCode() {
		/* 방법1 : 정수형변수와 객체의 해시코드값을 합산한 후 그대로
		  반환한다. */
		int returnCode1 = super.getAge() + 
				this.subject.hashCode();
		 System.out.println("returnCode1="+ returnCode1);
		 
		 /* 방법2 : 방법1이 번거롭다면 Object 클래스의 정적메서드인
		   hash()를 사용해도 된다. 멤버변수의 갯수만큼 인수로 전달하면
		   위와 동일하게 고유한 주소값을 정수형으로 반환해준다. */
	...
	/*
	equals()
	: 객체가 가진 멤버변수의 값을 비교한다. 값이 같을때는 true 다를때는
	  false를 반환하도록 오버라이딩한다. 
	*/
	@Override
	public boolean equals(Object obj) {
		...
		if((teacher.getAge()==super.getAge())
			...
			/* 모든 내용이 일치하면 true를 반환한다. 그러면 set컬렉션에는
			  add(추가)되지 않는다. */
			return true;
		}
		else {
