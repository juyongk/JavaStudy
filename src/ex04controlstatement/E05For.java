...
public class E05For {
	...
	public static void main(String[] args) {
		...
		//변수 i가 1부터 5까지 1씩 증가하므로 5번 반복된다.
		...
		*/
		/*
		방법1 : 1~10까지의 10번 반복해서 if문을 통해 2의 배수를
			찾은 후 누적해서 더한다.
		*/
		int total = 0;
		for(int i=1 ; i<=10 ; i++) {
		...
		System.out.println("1~10사의 2의 배수의합(방법1):"+ total);
		...
		*/
		...
		System.out.println("1~10사이 2의배수의 합(방법2):"+ total);
		...
		for(;;) {
			...
			a++;
			//1000이 되면 탈출하는 조건을 추가한다.
			if(a==1000) break;
		}
		
		/*
		for문의 끝에 ;을 사용하면 반복할 문장이 없는것으로 
		간주되어 for문과 상관없이 한번만 실행된다.
		*/
		for(int j=0 ; j<=5 ; j++);
		{
		...
		(Local variable)라고 한다. 지역변수는 해당 지역을 벗어나면
		즉시 소멸되는 특징을 가지고 있다.
		*/
