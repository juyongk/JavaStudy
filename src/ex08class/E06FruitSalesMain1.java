...
class FruitSeller{
	...
	/* 멤버변수의 경우 초기값이 없는 상태로 정의한 후 인스턴스를
	   생성하고 차후 초기화 할 수 있다.
	   하지만 멤버상수는 인스턴스 생성과 상관이 정의하는 시점에
	   반드시 초기화 해야한다. 즉 1000을 지우면 에러가 발생한다.*/
	
	//멤버 메서드
	/*
	판매자가 사과를 판매하는 행위를 표현한다.
	1.구매자로부터 금액을 전달받는다.(메서도 호출시 매개변수를 통해 받는다.)
...
class FruitBuyer{
	...
	1. 판매자에게 금액을 지불한다.
	=> 여기에서는 seller객체의 saleApple()메서드가 호출된다.
	2.판매자가 금액에 해당하는 사과의 갯수를 반환하면, 해당
	갯수만큼 보유한 사과가 증가한다. 
	=> numOfApple += 메서드호출();
