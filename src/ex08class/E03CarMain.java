...
class Car{
	...
	/* 우리가 직접 작성한 클래스는 객체를 표현하기 위한 용도로
	  ...
	  표현하는 또 하나의 자료형을 생성하는것이 class를 작성하는
	  목적이 된다.*/
	
	/* 초기화메서드1 : 매개변수가 없는 형태로 항상 고정된 값으로만
	   인스턴스를 초기화할수있다. 따라서 확장성이 없는 형태의 
	   메서드이다. */
	void initialize() {
		//Car클래스의 멤버변수를 초기화한다.
		carModel = "람보르기니";
		/* 자동차 소유주를 표현한 Human타입의 멤버변수는 아직
	...
	void showCarInfo() {
		...
		/* 소유주를 표현한 owner객체의 멤버메서드 호출을 통해
...
public class E03CarMain {
	...
	public static void main(String[] args) {
		...
		*/
		...
		/*
		자동차객체2 생성 : 초기화 메서드를 호출하는 대신 멤버변수에
		 직접 접근하여 인스턴스를 초기화한다. 이 경우 인스턴스를 
		 생성할때마다 동일한 코드를 작성해야 하므로 중복이 많아지게
		 되어 비효율적이다. 따라서 이 방법은 거의 사용하지 않는다.
		*/
		Car car2 = new Car();
		car2.carModel = "밴틀리";
		...
		/*
		자동차객체3 생성 : 매개변수가 있는 초기화 메서드를 사용하여
		 인스턴스를 생성한다. 이 경우 매개변수를 통해 다양한
		 값을 전달할 수 있으므로 여러형태의 객체를 생성할 수
		 있다. 따라서 가장 효율적인 코드라 할수있다.
		*/
		Car car3 = new Car();
