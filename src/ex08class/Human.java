...
public class Human {
	...
	int age;
	//에너지는 증가 혹은 차감이 필요하므로 반드시 int로 설정해야 한다.
	int energy;
	
	//멤버메서드 : 객체의 행위를 표현한다.
	void walking() { //'걷는다'를 표현
		energy--;
		/* 걷는 행위는 에너지를 1 차감시킨다. 만약 에너지를 차감한
		   결과가 음수라면 해당 시나리오의 조건에 위배되므로 0으로
		   고정하는 코드가 포함 되어있다.*/
		if(energy<0) {
			//차감 후 만약 음수가 되면 0으로 고정한다.
			energy = 0;
			System.out.println("[walk]에저지는 0이 최소입니다.");
