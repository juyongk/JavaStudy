...
정의된 클래스이다.
이와같이 데이터를 저장하기 위한 용도의 클래스를 가르켜 VO(Value Object)라고 한다.
즉 값만 가진 객체라는 뜻이다.
*/
class Friend {
...
class HighFriend extends Friend {
	...
	/* 오버라이딩1 : 고딩친구의 전체정보를 출력하기 위해 부모에서 정의한
	  메서드를 super를 통해 호출하고, 자식에서 확장한 변수를 추가해서
	  출력한다. */
	@Override
	public void showAllData() {
...
public class E10MyFriendInfoBook {
	/* 메뉴를 출력하는 용도의 메서드로 매개변수, 반환타입이 
	  없는 형태로 정의한다. main메서드에서 직접 호출하기 위해 static으로
	  선언되어있다. */
	...
	/*
	무한루프 조건으로 사용자가 워낳ㄹ때 종료할 수 있는 구조를 만들어준다.
...
class FriendInfoHandler {
	...
	/* 상속관계에서 부모클래스로 인스턴스 배열을 생성한다. 부모로
	  자식을 참조할 수 있으므로, 부모타입의 배열에는 자식타입의 
	  인스턴스를 저장할 수 있다. */
...
//		System.out.println("addFriend 호출됨");
		//고딩 or대딩 모두 기본정보가 있으므로 먼저 입력받는다.
		Scanner scan = new Scanner(System.in);
		...
		if(choice==1) {
			...
			/* 참조변수를 인스턴스 배열에 추가한다. 인덱스로 사용된 변수의 초기값이
			 0이므로, 0번 인덱스에 추가한 뒤 1증가 시킨다. 이를 위해 '후위증가' 하고있다. */
		...
		/* 더 이상 실행할 문장이 없으므로 해당 메서드는 메로리에서 소멸되고,
		  호출한 지점인 main메서드로 돌아간다.  */
	}///end of addFriend
	...
	/*
	 ...
	 4. 출력을 위한 멤버메서드는 상속관계에서 오버라이딩 처리되어 있으므로
	 	참조변수의 영향을 받지 않고 항상 자식클래스의 오버라이딩 된
	 	메서드가 호출된다.
	...
	*/
	...
	public void showSimpleData() {
		...
		자식클래스에서는 오버라이딩을 할 수 없으므로 개별적인 멤버메서드를 
		정의해야 한다.
		정보가 저장되는 배열은 Friend타입이므로 자식인스턴스를 저장하면
		자동으로 형변환(업캐스팅)된다. 이때 자식의 정보를 출력하는 메서드를
		바로 호출할 수 없으므로 아래와 같이 일일이 확인 한 후
		강제형변환(다운캐스팅)해야 한다.
		
		for(int i=0 ; i<numOfFriends ; i++) {
			for(int=0 ; i<numOfFriends ; i++) {
	...
	public void searchInfo() {
		...
		for(int i=0 ; i<numOfFriends ; i++) {
			/* 배열의 각 인덱스에 저장된 인스턴스의 참조값을 통해 멤버변수에
			  접근한다. 검색을 위해 입력한 이름과 비교해서 일치하는 경우에만
			  정보를 출력한다. 이때 equals()를 사용해도 결과는 동일하다. */
...
//		System.out.println("deleteInfo 호출됨");
		...
		/* 배열의 원소중 삭제된 원소의 인덱스값을 저장하기 위한 
		  변수로 삭제한 후 빈자리를 채워넣을때 사용한다. 인덱스는
		  0부터 시작이므로 -1을 초기값으로 설정한다. */
		int deleteIndex = -1;
		
		//삭제한 데이터를 찾기위해 저장된 갯수만큼 반복한다.
		for(int i=0; i<numOfFriends ; i++) {
			//입력된 이름과 일치하는지 확인한다.
			if(deleteName.compareTo(myFriends[i].name)==0) {
				/* 인스턴스 배열에서 삭제는 null로 변경하면 된다.
				 참조값이 null이라는 것은 참조할 인스턴스가 없다는 
				 의미이므로 삭제한것과 동일한 결과가 된다.*/
				myFriends[i] = null;
				//삭제된 원소의 index를 저장한다.
				deleteIndex = i;
				//전체 카운트를 1차감한다.
		...
		if(deleteIndex==-1) {
			...
			/* 인스턴스 배열에서 원소를 삭제한 후 바로 뒤에 있는
			  ...
			  않는다면 차후 검색이나 삭제시 NullPointerException이
			  발생할 수 있다. */
