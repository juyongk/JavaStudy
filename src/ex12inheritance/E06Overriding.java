...
오버라이딩(Overriding)
 ...
 되므로 항상 하위클래스에서 오버라이딩한 메서드가 호출된다. 
*/
//부모클래스 : 일반적인 스피커를 추상화한다.
class Speaker {
	//멤버변수 : 볼륨을 표현
	private int volumnRate;
	//멤버변수가 private이므로 외부에서 설정할 수 있도록 setter를 정의한다.
...
class BaseSpeaker extends Speaker {
	...
	/* @Override : 이노테이션이라 부르고, 오버라이딩 된 메서드에 추가하여
...
public class E06Overriding {
	...
	public static void main(String[] args) {
		...
		/* 단 부모타입의 변수로 자식 인스턴스를 참조하면 접근할 수 
		  있는 범위가 부모인스턴스 까지로 제한되므로, 자식쪽에 정의되어
		  있는 setBase()는 호출할 수 없게된다. */
