...
오버로딩 : 메서드명은 같지만 매개변수의 갯수, 타입, 순서가 다른
	경우를 말한다. 즉, 서로 다른 메서드이므로 상속받은 하위클래스
	에서는 오버로딩 된 모든 메서드가 포함된다.
...
class C extends B {
	...
	/* 따라서 C클래스로 인스턴스를 생성하면 오버라이딩된 메서드는
	  모두 가려지게 되므로 C클래스에 정의된 메서드 하나만 남게된다.
...
public class E07RideAndLoad01 {
	...
	public static void main(String[] args) {
		...
		/* 참조변수에 상관없이 항상 최하위에 오버라이딩된 메서드가 
		  호출된다. 즉 아래부분은 모두 C클래스에 정의된 rideMethod가
		  호출된다.
...
//		C refId3 = refId1;
		/*refId1은 C인스턴스를 참조하지만 원래 A타입이므로
		  C타입이라 착각하면 안된다. */
//		
		/* 자식으로 부모인스턴스를 참조하는 것인 기본적으로 불가능하다.
		  하지만 다운캐스팅(강제형변환)을 하면 참조가 가능해진다.
		  즉 부모타입을 자식타입으로 변경하는 것이다. 이는 클래스가
		  상속관계에 있기 때문에 가증하고, 방식은 기본자료형과 동일하게
		  소괄호를 이용하면 된다. */
//		B refId2 = (B)refId1;
//		C refId3 = (C)refId1;
	}
