...
따라서 실제 업무에서도 유사한 기능의 클래스들을 상속으로 묶어서 
정의한다. 
*/
...
class Computer{
	...
	/* 개발자가 임의로 생성자를 추가하면 디폴트 생성자는 추가되지
	않는다. */
	...
	/* private 접근지정자로 선언된 멤버메서드는 외부 클래스
	 혹은 상속받은 클래스에서 접근이 불가능하므로, public으로
	 선언된 메서드를 통해 간접적으로 호출해야 한다.
	*/
	...
	/* private으로 선언된 멤버메서드를 외부에서 호출할 수 있게
	  public으로 지정하여 선언한 메서드로 '캡슐화'를 적용하였다. */
...
/* 태블릿 노트북 : 컴퓨터의 기본 연산기능과, 휴대이용 그리고 전용펜으로
...
class TabletNotebook extends NotebookComputer{
	...
	//태블릿 사용을 위해 배터리량 체크 및 등록된 펜인지 확인
	public void write(String penInfo) {
		...
		/* 부모클래스에 정의된 메서드를 호출한다. 상속받은 자식
		  클래스에서는 이와 같이 부모의 멤버메서드를 호출하여 자신의
		  기능처럼 사용할 수 있다. 단 접근이 가능한 지정자로 선언
		  되어야 한다. super는 생략할 수 있다.*/
		super.movingCal();
...
public class E04ISAInheritance {
	...
	public static void main(String[] args) {
		...
		System.out.println("==ISE-1234 펜으로 태블릿사용==");
		tablet.write("ISE-1234");
		
		System.out.println("==XYZ-9876 펜으로 태블릿사용==");
