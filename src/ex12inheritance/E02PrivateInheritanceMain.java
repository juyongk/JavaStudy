...
class Account{
	/* 멤버변수가 상속관계에 있다 하더라도 private으로 선언되면
	  클래스 내부에서만 접근할 수 있다. 즉 상속받은 하위클래스 에서는
	  직접 접근이 불가능하다. */
	private int money;
	
	/* 이와 같이 개발자가 생성자를 직접 정의하면 디폴트 생성자는 추가
	  되지 않는다. 디폴트 생성자는 개발자가 생성자를 정의하지 않았을때만
	  컴파일러에 의해 자동으로 추가된다.  */
	...
	//private멤버변수를 외부로 반환할때 사용하는 메서드(getter)
...
class SavingAccount extends Account {
	...
	public SavingAccount(int initVal) {
		/* 만약 아래 문장을 삭제하면 즉시 에러가 발생한다. 삭제하는
		  ...
		  호출하는 문장이 자동으로 추가되는데, 부모클래스에는 이런
		  형태의 생성자가 없으므로 아래와 같이 명시해야 한다. */
		super(initVal);
	}
	/* 부모클래스에 정의된 멤버변수 money는 private이므로 
	  자식쪽에서는 아래와 같이 접근할 수 없다. 보이지 않으므로
	  에러가 발생한다. 따라서 부모클래스에 protected로 선언된
	  멤버메서드를 통해 입금처리를 해야한다. */
	public void saveMoney(int money) {
//		super.money += money;
		depositMoney(money);
	}
	/* 잔고를 확인하기 위한 멤버메서드로 부모의 멤버변수인 money에
	  직접 접근할 수 없으므로 아래와 같이 getter를 통해 반환받아
	  출력해야 한다. */
...
//		sa.money = 100000; //에러발생
		...
		/* 부모클래스를 통해 객체를 생성한다. 하지만 해당 인스턴스
		  ...
		  현재 접근하려는 위치가 클래스 외부이기 때문이다. 즉, private
		  멤버는 무조건 클래스 내부에서만 접근을 허용한다. */
