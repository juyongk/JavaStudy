...
형식]
	...
	※부모클래스를 상속받아 이름없는 자식클래스를 생성하는 개념으로 이해하면
	 된다. 상속되기 때문에 오버라이딩이 가능하다. 즉, {} 안쪽은 "자식"의
	 영역이다.
*/
//가수를 표한한 부모클래스
class Singer {
	...
	/* Object클래스에 정의된 메서드로 객체가 가진 값을 문자열로 반환해주는
	 메서드이다. Java에서 정의하는 모든 클래스는 Object 클래스를 상속
	 하므로 이와같이 오버라이딩이 가능하다.
	*/
	@Override
...
/* 인터페이스 정의 : 기능이 없는 추상메서드만 멤버메서드로 정의할수 있다.
	또한 메서드 앞에서는 public abstract가 생략되어있다. */
interface ISinger {
	public void dancing();
}
public class E02AnonymousClass {
	...
	public static void main(String[] args) {
		...
		/* 인스턴스의 참조값을 할당받은 참조변수를 호출한다. 이때
		  오버라이딩한 toString()메서드의 반환값이 출력된다.
		...
		: Singer타입의 변수에 GirlGroup을 상속받은 익명클래스를 생성
		해서 그 주소값을 할당한다.
		개념적으로 new GirlGroup(){}; => 익명 extends GirlGroup과 같다.
		이름이 없는 인스턴스이므로 부모클래스의 이름을 빌려서 자식의 영역을
		만든다고 생각하면 된다.
		*/
		Singer s3 = new GirlGroup("아이린","레드벨벳") {
			...
			/* 부모의 참조변수로 자식을 참조할 수 있지만, 접근범위가
			  ...
			  멤버변수(boolean)은 접근할 수 없어 에러가 발생한다.
			  접근하고 싶다면 다운캐스팅(강제형변환)을 해야하지만 익명
			  클래스는 이름이 없기때문에 다운캐스팅 자체가 불가능하다.
