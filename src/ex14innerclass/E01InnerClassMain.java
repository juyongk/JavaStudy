...
-정적 멤버를 가질수없다. 단 상수는 가질수 있다.
-컴파일시 외부 클래스명$내부클래스명.class 파일이 생성된다.
...
public class E01InnerClassMain {
	...
	public static void main(String[] args) {
		...
		/* 외부클래스의 인스턴스를 먼저 생성한 후 내부클래스의 인스턴스를
		  생성해야 한다. Marine은 Unit클래스의 생성자에서 이미 인스턴스
		  생성이 되어있으므로 아래와 같이 사용할 수 있다. */
		Unit.Marine marine = new Unit("마린", 50).marine;
		marine.attack();
		
		/* Medic의 경우 Unit의 생성자에서 인스턴스 생성이 되지
		  않았으므로 아래와 같이 new를 통해 생성해야 한다. 이때에도
		  Unit 인스턴스를 통해 생성해야 하므로 new를 두번 사용해서
		  인스턴스를 생성한다. */
