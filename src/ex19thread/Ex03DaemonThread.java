...
메인쓰레드가 종료되어도 계속 실행되며, Dead상태가 되어야만 실행이 종료되는
쓰레드를 말한다. 해당 쓰레드는 main() 메서드가 종료된 이후에도
계속 실행되는것을 볼수있다. 
*/
class NormalThread extends Thread{
	...
	/* 쓰레드 동작중 비정상적으로 종료될때 InterruptedException이 발생
	할 수 있으므로, 예외처리를 해야한다. 여기서는 즉시 처리하지않고,
	throws해서 호출한 위치로 예외객체를 던지기한다. 
	*/
	void threadMethod() throws InterruptedException{
		/* 10번 반복중 2초씩 block 상태로 돌아가므로 총 20초간 반복된다.
		  반복횟수와 쓰레드명을 출력한다. */
		for(int i=1 ; i<=0 ; i++) {
...
주로 프로그램의 보조역할로 배경음악재생, 로그자동저장등의 업무를
처리한다. 프로그램이 종료될때 함께 종료되므로 무한루프로 구현한다. 
*/
...
public class Ex03DaemonThread {
	...
	public static void main(String[] args) {
		/* 쓰레드로 구현하지 않는 일반적인 클래스의 작업으로 순차적으로
		  진행되어 1번 인스턴스가 작업을 마쳐야만 2번째가 실행된다. */
		NormalClass nt1 = new NormalClass("첫번째 클래스");
		nt1.ClassMethod();
		new NormalClass("두번째 클래스").ClassMethod();
		
		/* 첫번째 독립쓰레드 생성 및 시작. 우선순위를 별도로 부여하지 않으므로
		  5로 지정된다. 쓰레드 인스턴스 생성시 이름을 부여하는 방법으로
		  아래 2가지가 있다. */
		//방법1 : 생성자를 통해 쓰레드의 이름부여
		NormalThread yt1 = new NormalThread("1st 쓰레드");
		//방법2 : setName()을 통해 쓰레드의 이름 부여
//		yt1.setName("First쓰레드");
		yt1.start();
		...
		//Thread클래스의 정적메서드를 통해 쓰레드의 여러상태를 확인할 수 있다.
		System.out.println("현재 활성화된 상태의 쓰레드수"
				+ Thread.activeCount());
