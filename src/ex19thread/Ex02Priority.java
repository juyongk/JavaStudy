...
class MessageSendingThread extends Thread{
	...
	//성상자2 : 이름과 우선순위까지 지정한다.
	public MessageSendingThread(String str, int pri) {
		message = str;
		setPriority(pri);
	}
	public void run() {
		/* 각 쓰레드는 1000번씩 반복출력한다. 이때 부여받은 우선순위를 출력한다. */
		for(int i=1 ; i<=100 ; i++) {
			System.out.println(message + i +"("+getPriority() +")");
			/* 기본적으로 우선순위가 높은 쓰레드가 먼저 실행되지만
			 sleep()을 호출하면 실행중 잠깐씩 블럭상태로 전환되어
			 우선순위가 낮은 쓰레드도 실행될수있는 기회가 생기게된다.
			*/
			try {
				sleep(1);
			}
...
public class Ex02Priority {
	...
	public static void main(String[] args) {
		...
		동일한 우선순위 5를 부여받게 된다. 이때는 CPU가 알아서 작업을
		분배한다. 
		*/
