...
-하지만 개발자가 정의한 클래스라면 hashCod(), equals()메서드를
...
public class Ex04HashSet1 {
	...
	public static void main(String[] args) {
		...
		/* Object를 기반으로 set컬렉션을 생생했으므로 모든 객체를
		  저장할 수 있다. */
		...
		int varInt = 100;//저장시 Integer로 Boxing처리됨.
		...
		*/
		System.out.println(set.add(new String("개발자")) ? 
				"저장성공":"저장실패");
		//저장에 실패했으므로 5개가 출력된다.
		...
		이터레이터를 통한 객체 출력 : 순서없이 저장되므로 출력의 순서도
			정할 수 없다.
		*/
		//1.set참조변수를 통해 Iterator객체를 생성한다.
		...
		while(itr.hasNext()) {
			...
			/* HashSet<E>에 저장시 모두 Object형으로 자동형변환되어
			  저장되므로 우리가 직접 정의한 클래스에 대해서만 instanceof를
			  통해 타입을 확인한 후 다운캐스팅하여 출력한다. 만약 다운캐스팅을
			  하지 않으면 자식멤버에 접근할 수 없으므로 getName()을 호출할
			  수 없다.*/
