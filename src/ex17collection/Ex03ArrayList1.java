...
public class Ex03ArrayList1 {
	...
	public static void main(String[] args) {
		...
		/* String인스턴스를 저장할 수 있는 List컬렉션 생성
...
//		Vector<String> list = new Vector<String>();
		...
		*/
		...
		list.add("트와이스");
		list.add(3,"워너원");
//		list.add(5,"소방차"); //에러발생됨
		/*
		저장된 객체수(리스트의 크기) : size()
			컬렉션에 저장된 인스턴스의 갯수를 반환한다. 배열에서
			사용하는 length와 동일한 값을 반환한다. 
		*/
		...
		중복저장 : List는 배열의 특성을 가지므로 데이터를 중복저장
			할수있다. 동일한 데이터라도 index로 구분할 수 있기 
			때문이다. add()는 추가에 성공할 경우 true를 반환한다.
		*/
		...
		방법1 : 일반 for문 사용. 이때는 인덱스를 통해 접근해야 하므로
			get()메서드를 사용한다. 
		*/
		System.out.println("\n[일반for문 사용]");
		...
		/*
		방법2 : 확장 for문의 경우 인덱스없이 전체 원소를 반복해주므로
			좀 더 간단한 표현이 가능하다. 따라서 가장 많이 사용된다. 
		*/
		System.out.println("\n[확장for문 사용]");
		for(String obj : list) {
		...
		/*
		방법3 : Iterator 사용
			1.컬렉션에 저장된 내용을 Iterator 객체에게 알려주기
			위해 인스턴스를 생성한다. 
			2.hasNext()로 출력할 인스턴스가 있는지 검사한다. 만약
			인스턴스가 더이상 없다면 false를 반환한다.
			3.앞에서 확인후 next()를 통해 인스턴스를 출력한다.
		*/
		System.out.println("\n[반복자(Iterator) 사용]");
		...
		*/
		...
		삽입하기 : add(삽입할 인덱스, 인스턴스)
			해당 인덱스에 인스턴스를 삽입한다. 기존 인스턴스는 뒤로
			1칸씩 밀려난다. 즉, 자동으로 인덱싱된다. 
		*/
		...
		*/
		...
		방법1 : 인덱스로 삭제
			remove(인덱스) : 인덱스를 통해 삭제를 진행한 후 삭제가
				완료되면 해당 인스턴스를 반환한다. 삭제 후에는
				인덱스가 자동으로 부여된다. 삭제 성공시 해당 인스턴스를
				반환하므로 삭제 후 즉시 확인할 수 있다.  
		*/
