...
public class Ex03ArrayList2 {
	...
	public static void main(String[] args) {
		...
		/* 아래2개의 컬렉션은 내부적인 자료구조만 다를뿐 구현방법에는 차이가
		  없다. 인스턴스 생성시 Student형의 타입매개변수를 통해 생성한다.
		  즉, Student 인스턴스를 저장할 수 있는 상자가 된다. */
		LinkedList<Student> list2 = new LinkedList<Student>();
//		ArrayList<Student> list2 = new ArrayList<Student>();
		
		/* 저장할 Student인스턴스 생성. 외부패키지에 선언되었으므로 반드시
		  import후 사용해야 한다. */
		...
		/* 일반 for문 사용을 위해 size()메서드로 추가된 인스턴스의 
		  갯수를 반환받는다. 또한 index로 접근해야 하므로 get()
		  메서드를 사용한다. */
		for(int i=0 ; i<list2.size() ; i++) {
			/* List의 각 인덱스에 저장된 인스턴스의 참조값을 그대로
			  출력한다. toString() 메서드를 오버라이딩 했으므로
			  객체에 저장된 멤버변수값이 출력된다. 만약 오버라이딩이
			  되어있지 않다면 참조값이 출력될것이다. */
			System.out.println(list2.get(i));
		}
		
		/* 인스턴스의 참조값을 통해 삭제하는 경우 boolean값이 반환된다.
		  또한 동일한 인스턴스가 존재한다면 앞에 있는것이 먼저 삭제된다. 
		  즉 동시에 삭제되지 않는다. */
		...
		int idx = 0;
		/*
		시나리오] 컬렉션에 저장된 객체를 이름으로 검색하여 삭제할 수 있는
		코드를 작성하시오. 이때 반복문은 확장 for문을 사용하시오.
		*/
		...
		for(Student st : list2) {
			/* 이름으로 검색시 멤버변수 name은 private으로 선언되었으므로,
				getter를 통해 접근해야한다. 삭제를 위해 입력한 이름과
				Student객체에 저장된 name과 일치하는지 확인한 후 해당
				인스턴스를 찾을 수 있다.
			*/
