...
public class Ex01GenericBasic {
	...
	public static void main(String[] args) {
		...
		/* 오렌지박스 인스턴스아ㅗ 당도가 10인 오렌지 인스턴스를
		  생성한 후 저장한다.*/
		...
		orange1.showInfo();
		/* 상자1은 오렌지 전용박스이므로 다른과일(즉 객체)를 저장할 수 
		 없다. 컴파일 에러가 발생하므로 실행자체가 불가능하다.
		 즉 자료형에는 안전하지만, 구현에는 불편함이 있다. 다른 객체를
		 저장하기 위해서는 또 다른  Box클래스를 생성해야 한다. */
		
		/* 아래와 같이 Orange가 아닌 객체를 저장하면 컴파일 에러가
		 발생하여 실행할 수 없는 코드가 된다. 즉 자료형에 
		 안전하다고 할수있다. */
//		oBox1.store("당도가 20인 오렌지");
		...
		/* 하지만 오렌지 같은 String을 저장하고, 이를 꺼내서
		  다운캐스팅 하는 부분에서는 예외가 발생된다. 즉 런타임 에러가
		  발생하게 되므로 실행해보기 전에는 오류를 예측하기 쉽지않은
		  단점이 있다. */
