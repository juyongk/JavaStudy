...
/* 프로젝트 전체에서 기준의 역할을 하는 추상클래스
  해당 클래스는 추상메서드를 포함하고 있으므로 하위에서 상속받게 되면
...
class PersonalInfoDTO {
	...
	/* getter/setter : DTO객체에서 private으로 선언된 멤버변수의 값을
	설정하거나 반환할때 사용한다. */
	...
	/* 제공된 추상클래스를 상속하여 한사람의 정보를 저장하는 기능과
	  검색의 기능을 구현한 클래스*/
class PersonalNumberStorageExt extends PersonalNumberStorageAbs {
	...
	/* 상속을 통해 추상메서드를 포함하게 되므로 반드시 아래와 같이 오버라이딩
	  해야한다. 이는 필수사항이며 하지 않을경우 에러가 발생한다.
	 */
	@Override
	...
	public String searchPersonalInfo(String juminNum) {
		...
		for(int i=0 ; i<numOfPerInfo ; i++) {
			...
			if(juminNum.compareTo(personalArr[i].getJuminNum())==0) {
				...
				/* DTO객체의 멤버변수는 private으소 선언되었으므로
				  외부에서는 접근이 불가능하다. 따라서 getter를
				  통해 접근해야한다. */
			}
		}
		//검색결과가 없다면 null을 반환한다.
		return null;
	}
}
public class E02AbstractToInterface1 {
	...
	public static void main(String[] args) {
		...
		//2개의 정보를 추가
		storage.addPersonalInfo("901234-2222222", "박희성");
		storage.addPersonalInfo("901234-1111111", "이준석");
