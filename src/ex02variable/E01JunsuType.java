...
public class E01JunsuType {
	...
	public static void main(String[] args) {
		...
		*/
		...
		-byte, short타입은 게임 캐릭터의 움직임이나 음원파일 등을
		표현할때 주로 사용된다. 
		*/
		...
		System.out.println("shotrt1 + short2 = "+ result3);
		
		/*
		-동일 자료형끼리의 연산은 동일자료형이 되는것이 기본원칙이다.
		-하지만 큰 자료형과 작은 자료형을 연산하면 큰 자료형으로
		자동 형변환된다. 이유는 데이터의 손실(오차)이 있을 수
		있기 때문이다.
		...
		*/
		...
		이떄 접미사 (L 혹은 l)를 붙여서 int형의 자료가 아닌
		long형의 자료임을 컴파일러에게 알려줘야 한다. 이와 같이
		할당을 위해 입력되는 숫자도 자료형을 기반으로 메모리에
		로드되는데 이를 가르켜 리터럴(literal)이라고 한다.
		*/
		//long long3 = 2200000000; //int는 21억까지 표현가능,에러발생
