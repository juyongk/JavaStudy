...
/*
싱글톤 디자인 패턴
: 클래스를 설계하는 디자인패터의 하나로써 하나의 인스턴스 즉 하나의
메모리를 할당한 후 이를 프로그램 전체에서 공유해서 사용하고자 할때
쓰는 패턴이다.

작성방법
	1. 생성자의 접근 지정자를 private으로 선언한다.
	2. 그러면 클래스 외부에서는 생성자에 접근할 수 없으므로 new를
		통해 인스턴스 생성이 불가능해진다.
	3.클래스 내부에 멤버변수를 static으로 선언하여 인스턴스를 미리 생성해 놓는다.
	4. getInstance()라는 유틸리티 메서드를 통해 참조값을 반환받아
		사용한다.
	5. 이 방법을 통해 인스턴스를 생성하면 메모리에 딱 하나만 만들어지게 되어
		메모리를 절약할 수 있다.
*/
class NoSingleTone{
	int instVar;
	/* 생성자는 거의 대부분 public으로 선언한다. 만약 public으로
	  선언하지 않으면 클래스 외부 혹은 다른 패키지에서는 호출할 수
	  없기 때문에 new를 통해 인스턴스를 생성할 수 없기 때문이다. */
	public NoSingleTone() {}
}
//싱글턴 패턴이 적용된 클래스 정의
class SingleTone{
	...
	/* 정의한 클래스와 동일한 타입으로 선언된 정적 멤버변수로
		JVM(자바가상머신)에 의해 프로그램이 시작될때 미리 메모리(Method)
		에 로드되어 사용할 준비를 마치게 된다. */
	private static SingleTone single = new SingleTone();
	/* 생성자를 private으로 선언하면 클래스 외부에서는 호출할 수 
	  없으므로 new를 통해 인스턴스를 생성할 수 없게된다. */
	private SingleTone() {}
	
	/* 정적메서드로 선언된 getInstance()를 통해 해당 인스턴스의
	  참조값을 외부로 반환한다. 이런 메서드를 "유틸리티 메서드"라고 한다.
	*/
