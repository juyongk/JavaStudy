...
Wrapper : 기본자료형의 데이터를 인스턴스화할때 사용하는 클래스로
	Boxing 혹은 Unboxing을 통해 기본자료형의 데이터를 인스턴스화
	한다. 뒤에서 학습할 제네릭 클래스에서 필수적으로 사용된다.
*/
public class E01WrapperClass1 {
	...
	public static void main(String[] args) {
		...
		/* JDK1.4 이전의 코딩형태
		 기본자료형을 박싱 처리한 인스턴스에 대해서 산술연산을
		 진행할때 언박싱을 위해 intValue()와 같은 메서드를 호출해야 한다.
		 현재 이와 같은 생성자는 Deprecated(디프리케이트)되었다.*/
		...
		오토박싱 혹은 오토언박싱이 지원되므로 기본자료형 변수를 선언하는것과
		동일하게 코딩할 수 있다. */
