	...
	*/
	...
	public boolean equals(Object obj) {
		/* 매개변수로 전달된 인스턴스를 Object로 받으면 업캐스팅이
		  되므로 다운캐스팅 이후에 비교를 해야한다.
		  이때 전달된 인스턴스가 MyClass형이 아니라면 비교의 대상이
		  될수 없으므로 상속관계 확인을 위해 instanceof 연산자를
		  사용한다. */
		if(obj instanceof MyClass) {
			/* 매개변수가 MyClass 타입이 맞다면 비교를 위해 다운캐스팅
			 한다. 그렇지 않으면 부모타입으로는 자식 멤버에 접근할 수 
			 없으므로 멤버변수 data에 대한 비교가 불가능해진다. */
...
public class E04Object3Equals {
	...
	public static void main(String[] args) {
		...
		/* 만약 오버라이딩 해놓은 equals()를 삭제한 후 비교하면 다르다는
		 결과가 나오게 된다. 즉 오버라이딩이 필요없는 경우는 기본클래스
		 일때만 가능하다. */
