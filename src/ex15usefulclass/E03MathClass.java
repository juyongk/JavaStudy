...
/* Java의 규칙 : java.lnag 패키지 하위의 모든클래스는 자동으로 
  import된다. 또한 정의하는 모든 클래스는 Object를 상속받게 
  되어있다. 
  따라서 java.lang패키지의 모든 클래스는 별도의 선언없이 사용
  할 수 있고, Object에서 제공하는 메서드는 오버라이딩이 가능하다. */
...
public class E03MathClass extends Object {
	...
	*/
	public static void main(String[] args) {
		...
		/* round의 경우 가장 가까운 정수쪽으로 올림/버림을
		  결정한다. x.5 이상이면 올림처리, 미만이면 버림처리를 한다.
		  따라서 3.4xxx라면 버림처리를 하게 된다. */
		...
		System.out.println("Math.round:"+ Math.round(-1.6));//버림: -2
		System.out.println("Math.round:"+ Math.round(-1.5));//버림: -1
		System.out.println("Math.round:"+ Math.round(-3.5));//올림: -3
		System.out.println("Math.round:"+ Math.round(-3.51));//올림: -4
		
		System.out.println("### 최대/최소 ###");
		System.out.println("Math.max:"+ Math.max(100, 99));
		System.out.println("Math.min:"+ Math.min(100, 99));
		
		/* 0~1 사이의 실수값을 반환한다. 따라서 정수값을 만들고 싶을때는
		  특정한 값을 곱해주면 된다. */
