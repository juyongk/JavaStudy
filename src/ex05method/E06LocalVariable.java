...
-Stack은 입구가 하나만 있는 컵과 같은 형태이며, '선입후출'의
...
public class E06LocalVariable {
	...
	public static void main(String[] args)
	{
		...
		/* main지역에서 num을 선언하면 그보다 좁은 지역인 if문
		   에서는 같은 이름의 변수를 선언할 수 없다. 큰 지역에서 이미
		   선언되어 메모리에 상주하고 있기 때문이다. */
//		int num = 9; //아래 코드에서 에러가 발생됨.
		...
		*/
		...
		/* if문의 블럭에서 선언된 변수 num은 해당 블럭을 벗어나는
		 순간 메모리에서 소멸되어 아래에서 num을 선언할 수 있다.*/

		int num = 100;
		System.out.println("main메서도지역:"+ num);
	...
	/* main에서 선언한 변수 num은 메모리에 아직 상주하고 있지만
	  해당메서드는 크기가 동일한 서로 다른 지역에 정의되었으므로
	  같은 이름의 변수를 선언할 수 있다. 
	  앞의 main메서드의 if~else문의 블럭도 같은 개념으로 변수를 
	  선언한 것이다. */
