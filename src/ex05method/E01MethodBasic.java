...
-메서드 호출 후 반환값은 호출한 위치로 반환한다. 이때 메서드는
	메모리에서 소멸된다.
-반환값이 없는 void메서드도 실행이 완료되면 호출한 위치로
	실행의 흐름이 돌아온다.
*/
public class E01MethodBasic
{
	/*
	메서드1 : 반환값O, 매개변수O 형태의 정의
	...
	*/
	...
	메서드2 : 반환값X , 매개변수X 형태로 정의
		...
		메서드를 호출한다. 이와같이 반환값이 없는 경우에는 반드시
		void를 명시해야 한다.
	*/
	...
	/*
	메서드3 : 반환값X, 매개변수O 형태의 정의
		=> 문자열을 인수로 전달받아 단순히 출력만한다. 반환값이
		없는 경우에는 실행이 종료되면 호출했던 지점으로 실행의
		흐름이 이동한다.
	*/
