...
/*
함수형 인터페이스
: 람다식을 사용하기 위해서는 인터페이스를 먼저 만들고 람다식으로 구현할 추상
 메서드를 선언한다. 이를 람다식을 만들기 위한 함수형 인터페이스라고 한다.
 함수형 인터페이스는 오직 하나의 추상메서드만 선언할 수 있다. 
*/

/*
@FunctionalInterface : 함수형 인터페이스의 조건을 갖추었는지에 대한 
	검사를 컴파일러에게 요청하는 역할을 한다.
*/
@FunctionalInterface
interface ISchool4 {
...
public class Ex04FunctionalInterface {
	...
	/* main() 에서 정의한 람다식의 참조변수는 ISchool4타입이므로 
	  아래와 같이 매개변수로 전달받은 후 추상메서드인 studyLambda()를
	  호출할 수 있다. */
	static void excute(ISchool4 school4, String s) {
		/*
		school4 => 메인에서 선언한 람다식을 받은 매개변수
		s => 전달된 문자열
	...
	public static void main(String[] args) {
		...
		추상메서드를 통해 매개변수의 타입을 유추할 수 있으므로 아래와 같이
		String을 생략할 수 있다. 
		*/
		ISchool4 sch = (str) ->
